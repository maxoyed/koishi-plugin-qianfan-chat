...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.union([
    Schema.object({
      OPEN_HISTORY: Schema.const(true)
        .required()
        .description("是否开启多轮对话"),
      HISTORY_ROUND: Schema.number()
        .default(10)
        .description("多轮对话最大轮数"),
      ROUND_DURATION: Schema.number()
        .default(60)
        .description("自动结束会话间隔/秒"),
    }).description("多轮对话配置"),
    Schema.object({}),
  ]),
]) as any;
...
export function apply(ctx: Context, config: Config) {
    ...
    .action(async ({ session }, prompt: string) => {
      ...
      if (prompt && config.OPEN_HISTORY) {
        const minCreateTime = new Date(
          new Date().getTime() - config.ROUND_DURATION * 1000
        );
        // 查询上一次对话
  ...
  if (config.OPEN_IMAGINE) {
    ctx
      .command("imagine [prompt:rawtext]", "绘画")
      .action(async ({ session }, prompt: string) => {
        try {
